          ...
          {historicalMetrics && (
            <div className="p-4 bg-white rounded shadow-md mt-4">
              ...
              <Graph
                ...
                xAxis="timestamp"
                yAxis={["cpu_usage", "memory_usage", "disk_usage"]}
                type="line"
              />
            </div>
          )}
          <div className="p-4 bg-green-50 rounded shadow-md mt-4">
                ...
                {recommendations.map((rec, index) => (
                  <li key={index} className="mt-1">
                    ...
                    {rec.current_value > rec.recommended_value ? (
                      <>
                        Reduce from <strong>{rec.current_value}%</strong> to{" "}
                        <strong>{rec.recommended_value}%</strong>.{" "}
                        {rec.potential_savings > 0 && (
                          <>
                            Potential savings:{" "}
                            <strong>{rec.potential_savings}%</strong>.
                          </>
                        )}
                      </>
                    ) : (
                      <>
                        Increase from <strong>{rec.current_value}%</strong> to{" "}
                        <strong>{rec.recommended_value}%</strong> for better utilization.
                      </>
                    )}
                    <br />
